...
var client_instance = function (args) {
    ...
    self.indices.status = function (args, cb) {
        ...
        var _path = path.join(_index, '_status');

        var req_args = {
            url: self.url + _path
        };

        http_client.get(req_args, function (err, res, body) {
    ...
    self.indices.create = function (args, cb) {
        ...
        if (args.options) {
            req_args.body = JSON.stringify(args.options);
        }

        http_client.put(req_args, function (err, res, body) {
            response_handler.handle(err, body, cb);
        });
    };
    ...
    self.indices.del = function (args, cb) {
        ...
        http_client.del(req_args, function (err, res, body) {
            response_handler.handle(err, body, cb);
        });
    };

    self.indices.refresh = function (args, cb) {
        if (typeof args === 'function') {
            cb = args;
            args = {};
        }
        ...
        args = args || {};
        var _path = path.join(args.index, '_refresh');

        var req_args = {
            url: self.url + _path
        };

        http_client.post(req_args, function (err, res, body) {
            response_handler.handle(err, body, cb);
        });
    };
